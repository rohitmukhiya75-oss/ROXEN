...
export const dashboardStats = catchAsyncError(async(req,res,next)=>{
  ...
  const totalRevenueAllTimeQuery = await database.query(`
    SELECT SUM(total_price) FROM orders WHERE sale_completed = TRUE  
    `);
  ...
  const orderStatusCountsQuery = await database.query(`
      SELECT order_status, COUNT(*) FROM orders WHERE sale_completed = TRUE GROUP BY order_status
      `);
  ...
  const todayRevenueQuery = await database.query(
    `
    SELECT SUM(total_price) FROM orders WHERE created_at::date = $1 AND sale_completed = TRUE
    `,
    [todayDate]
  );
  ...
  const monthlySalesQuery = await database.query(`
    ...
    FROM orders WHERE sale_completed = TRUE
    GROUP BY month, date
    ORDER BY date ASC
    `);
  ...
  const currentMonthSalesQuery = await database.query(
    `
      ...
      FROM orders 
      WHERE sale_completed = TRUE AND created_at BETWEEN $1 AND $2  
      `,
