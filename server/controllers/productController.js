...
export const fetchAllProducts = catchAsyncError(async (req, res, next) => {
    const {availablity, category, price, ratings,search} = req.query;
    ...
    } else if (availablity === "limited-stock") {
    ...
    const newProductsResult = await database.query(newProductsQuery);
     ...
     //QUERY FOR FETCHING TOP RATINGS PRODUCTS (rating >=4.5)
    const topRatedQuery=`
    SELECT p.*,
    COUNT(r.id) AS review_count
    FROM products p
...
`;
    ...
    const {rows} = await database.query(purchaseheCheckQuery, [req.user.id, productId]);
    if (rows.length === 0) {
...
export const fetchAIFilteredProducts = catchAsyncError(async (req, res, next) => {
    ...
    const result = await database.query(
        `SELECT * FROM products WHERE name ILIKE ANY($1) OR description ILIKE ANY($1) OR category ILIKE ANY($1)
        LIMIT 200;`,
        [keywords]
    );
    ...
    if (filteredProducts.length === 0) {
        return res.status(200).json({
            success: true,
            products: [],
            message: "No products found matching the prompt.",
        });
    }
   ...
   if (!aiResult.success) {
    return next(new ErrorHandler("AI response is empty or invalid.", 500));
}
